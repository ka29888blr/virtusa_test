...
public class ConversionRules {
	...
	private static final Map<Integer, Integer[]> ROMAN_SUBTRACTABLE_MAPPING = Collections
	...
	private static final Map<Character, Integer> ROMAN_TO_NUMERIC_MAPPING = Collections
	...
	/**
	 * checkLiteralCountValidity() method keeps the count of all repeatable and non
	 * repeatable Literals.
	 * 
	 */
		...
		if (checkIfLiteralPresent(NonRepeatingRomanNumerals, currentLiteral)) {
			...
			if (keyForValueContainingThree != '\0') {
				if (currentLiteral.equals(keyForValueContainingThree)) {
					System.err
							.println("Error : Roman Numeral " + currentLiteral + " cannot repeat 4 times successively");
					System.exit(0);
	...
	/**
	 * getKeyForValueContainingThree() method returns the key corresponding to value
	 * 3.This is used while checking the count of Literals that cannot be repeated
	 * more than 3 times.
	 * 
	 */
	...
	/**
	 * checks if currentLiteral is smaller than the previous one. This rule is
	 * applied when repeatable literals can occur 4 times only if the 3rd and 4th
	 * occurance has a smaller value between them.
	 * 
	...
	/**
	 * Applies the subtaction logic and checks if the element is subtractable by the
	 * other or not.
	 * 
	 * @param lastDecimal
	 * @param decimal
